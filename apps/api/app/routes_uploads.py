...
SUPPORTED_TYPE_MESSAGE = "Unsupported file type. Supported types: EPUB, HTML, TXT, MD."
...
def create_upload(
    ...
    job = IngestionJob(upload_id=upload.id, status="queued", created_at=now, updated_at=now)
    db.add(job)
    db.flush()
