...
R1 and R3 are already being addressed by the current API boundary work, because the new API boundary page covers route ownership, service seams, error mapping, response contracts, and the preflight and postflight wiki rule. Keep them in this roadmap so they stay visible as follow-up themes, but do not treat them as separate runtime work unless a later phase needs a narrower spec.

R2, R4, R5, R6, and R7 remain later follow-ups unless another feature has already completed them. R2 should wait for the API boundary note to stay stable. R7 stays a guardrail, not a queue redesign.

## R1 - API boundary note or wiki link

- **Recommended feature title**: API boundary note or wiki link
- **Priority**: P1
...
- **Proposed scope**: Keep the durable API boundary page linked from the wiki spine and make sure it stays the primary entry point for route ownership, dependency seams, public errors, response contracts, and test seams.
...
- **Suggested Spec Kit prompt seed**: Create a docs-only feature that keeps the API boundary note as the durable route and service contract, links it from the wiki spine, and preserves the thin cross-layer map.
...
- **Proposed scope**: Document the shared convention for 400, 401, 404, 413, 415, and safe server-failure handling, with examples taken from the current route modules.
...
- **Suggested Spec Kit prompt seed**: Create a docs-only feature that records the public error mapping convention for API routes and ties it to the current route examples.
...
- **Proposed scope**: Decide whether `chat_rag.py` should stay as a compatibility facade, be renamed, or be retired in a later cleanup while keeping the existing chat tests and answer boundary intact.
...
- **Proposed scope**: Add a lightweight checklist that route authors can apply before merging new or changed response models, especially for auth, sessions, uploads, records, and archives.
...
- **Proposed scope**: Add targeted ownership and contract coverage for records and archive routes if those surfaces expand or start accumulating new responsibilities.
...
- **Proposed scope**: Keep queue follow-up separate from API architecture planning and document the guardrail wherever the queue boundary is discussed.
